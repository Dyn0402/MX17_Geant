...
**State 2026-08-07, later:** T2b done and validated (§3). Running: the nominal comb-kernel production point on the **desktop** (ρ_s 1 MΩ/sq, d_k 75 µm, nx 3120 = 10 µm, ny 1024 = 48.75 µm, 61 log times; ~2.5 min per Y kernel), and the T7 `collect.py` merge on **lxplus** — run *there*, not locally, because the 56 raw seed JSONs are 19 GB and only the merged `aval_calib.json` needs to travel. Critical path is now **S1 export decision → T9**. The export decision has a hard number attached to it at last: a full grid point is ~2.5 GB (2 Y kernels at 1.56 GB + 40 X kernels at 0.98 GB, float32), so the 12-point scan is ~30 GB — more than the desktop's 20 GB free. Options, to settle when T9 says what it actually needs: (a) trim the Y kernel's y range from ±25 mm to ±12.5 mm, (b) keep only the parameter points the blind comparison needs rather than all 12, (c) compute on demand. Do not blanket-produce all 12 points before T9 exists.
